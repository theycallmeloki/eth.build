...
ChessBoard.prototype.loadPieces = function () {
    ...
    for (let s in this.status.board.squares) {
        ...
        if (piece) {
            ...
            let pieceAndLocation =
                notation +
                '@' +
                this.status.board.squares[s].file +
                '' +
                this.status.board.squares[s].rank;
        ...
        this.render(
            ...
            >
                <div style={{ position: 'absolute', left: 50, top: 20 }}>
                    ...
                    <div>
                        {this.status.isCheck ? 'CHECK' : ''}
                        {this.status.isCheckmate ? 'MATE!' : ''}
                    </div>
                    <div>
                        {this.status.isRepetition ? 'REPETITION' : ''}{' '}
                        {this.status.isStalemate ? 'STALEMATE' : ''}
                    </div>
                </div>
                <ReactChess
                    pieces={this.pieces}
                    onMovePiece={(piece, from, to) => {
                        ...
                        for (let m in this.status.notatedMoves) {
                            ...
                            let dest =
                                this.status.notatedMoves[m].dest.file +
                                this.status.notatedMoves[m].dest.rank;
                            let src =
                                this.status.notatedMoves[m].src.file +
                                this.status.notatedMoves[m].src.rank;
