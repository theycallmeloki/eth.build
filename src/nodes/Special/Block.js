...
Block.prototype.processTx = function (tx) {
    ...
    try {
        if ((!this.properties.requireTo || tx.to) && tx.from && tx.value) {
            ...
            if (
                this.properties.requireNonce &&
                parseInt(tx.nonce) != thisNonce
            ) {
...
Block.prototype.onExecute = function () {
    if (
        this.properties.lockOnValidBlock /*&& this.properties.validTxns*/ &&
        this.properties.validNonce &&
        this.properties.validHash
    ) {
        this.title_color = '#22aa66';
        this.properties.title = '#' + this.properties.number;

        this.blockToDoublecheckHash = {
            transactions: JSON.parse(this.properties.validTxns),
            nonce: this.properties.validNonce,
            parent: this.parent ? this.parent.hash : 0,
        };
    ...
    if (this.flags.collapsed) {
        ...
        for (let t in this.txns) {
            ...
            if (typeof tx.nonce != 'undefined') {
                nonceDisplay = '[' + tx.nonce + ']';
            }
            let tableCell = '';
            if (tx.hash) {
                tableCell = (
                    <TableCell style={rowStyle}>
                        <Blockies seed={tx.hash} size={8} scale={2} />
            ...
            if (this.showingTo) {
                toCell = (
                    <TableCell style={rowStyle}>
                        <Blockies
                            seed={
                                tx.to && typeof tx.to.toLowerCase == 'function'
                                    ? tx.to.toLowerCase()
                                    : ''
                            }
                            size={8}
            ...
            let valueCell = '';
            valueCell = <TableCell style={rowStyle}>{tx.value}</TableCell>;
                ...
                <StyledTableRow>
                    <TableCell style={rowStyle}>
                        <Blockies
                            seed={
                                tx.from &&
                                typeof tx.from.toLowerCase == 'function'
                                    ? tx.from.toLowerCase()
                                    : ''
                            }
                            size={8}
                        ...
                        />
                        <span style={{ marginLeft: 4 }}>
                            {tx.from && typeof tx.from.substr == 'function'
                                ? tx.from.substr(0, 8)
                                : ''}{' '}
                            {nonceDisplay}
                        </span>
            ...
            minedBlockDisplay = (
                <div
                    style={{
                        ...
                        left: 80,
                        top: 30,
                        width: this.size[0] / 1.7,
                    }}
                >
                    {this.properties.validHash
                        ? this.properties.validHash.substr(0, 8)
                        : ''}{' '}
                    {!this.properties.validHash && this.hash
                        ? this.hash.substr(0, 8)
                        : ''}{' '}
