...
Ledger.prototype.processTx = function (tx) {
    ...
    try {
        if (
            (!this.properties.requireTo || tx.to) &&
            tx.from &&
    ...
    if (this.flags.collapsed) {
        ...
        for (let t in this.txns) {
            ...
            if (this.properties.valueType == 'float') {
                ...
                valueCell = (
                    <TableCell style={rowStyle}>{value.toFixed(2)}</TableCell>
                );
            } else {
            ...
            rows.push(
                <StyledTableRow>
                    <TableCell style={rowStyle}>
                        ...
                        />
                        <span style={{ marginLeft: 4 }}>
                            {tx.from ? tx.from.substr(0, 8) : ''} {nonceDisplay}
                        </span>
                    </TableCell>
                    {valueCell}
                    {toCell}
        ...
        if (this.properties.difficulty > 0) {
            hashCell = <TableCell>Hash</TableCell>;
        }
        ...
        let dataCell = '';
        if (this.showingData) {
            dataCell = <TableCell>Data</TableCell>;
        }

        let gasCell = '';
        if (this.properties.showGas) {
            gasCell = <TableCell>Gas</TableCell>;
        }
        ...
        this.render(
            <div
                style={{
                    ...
                    transform:
                        'scale(' +
                        (this.graph.canvas ? this.graph.canvas.ds.scale : 0) +
                        ')',
                    ...
                    width: this.size[0] - 5,
                    height: this.size[1] - topPadding - 1 - 5,
                }}
            >
