...
app.get('/price', (req, res) => {
    ...
    if (
        !coinmarketcapBuffer[symbol] ||
        !coinmarketcapBufferTimestamp[symbol] ||
        coinmarketcapBufferTimestamp[symbol] + 30000 < Date.now()
    ) {
        console.log(
            'getting price data for ' +
                symbol +
                ' and reseting buffer time to ',
            coinmarketcapBufferTimestamp[symbol],
        );
        ...
        const requestOptions = {
            ...
            headers: {
                'X-CMC_PRO_API_KEY': fs
                    .readFileSync('coinmarketcap.key')
                    .toString()
                    .trim(),
            },
            json: true,
...
app.post('/build', (req, res) => {
    ...
    if (req.body && req.body.compressed) {
        ...
        console.log(
            'SAVE',
            req.body.compressed,
            'AND SAVE TO SOME RANDOM KEY AND RETURN IT',
            key,
        );
